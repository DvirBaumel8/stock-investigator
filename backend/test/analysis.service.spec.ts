...
import { AgentResult } from "src/agent-result/agent-result.entity";
...
import { Analysis, AnalysisStatus } from "src/analysis/analysis.entity";
...
describe("AnalysisService", () => {
  ...
  let analysisRepo: jest.Mocked<
    Pick<Repository<Analysis>, "findOne" | "save" | "update">
  >;
  let agentResultRepo: jest.Mocked<Pick<Repository<AgentResult>, "save">>;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: TechnicalAgent,
          useValue: { agentName: "technical", analyze: jest.fn() },
        },
        {
          provide: NewsSentimentAgent,
          useValue: { agentName: "news_sentiment", analyze: jest.fn() },
        },
  ...
  describe("createOrGetAnalysis", () => {
    ...
    it("normalizes ticker to uppercase", async () => {
      analysisRepo.findOne.mockResolvedValueOnce(null);
      analysisRepo.save.mockResolvedValueOnce(
        makeAnalysis({ ticker: "AAPL", status: AnalysisStatus.RUNNING }),
      );
