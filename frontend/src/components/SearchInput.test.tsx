...
describe('SearchInput — validation', () => {
  ...
  it('blocks Enter on unknown ticker and shows error', async () => {
    ...
    render(<SearchInput value="XYZXYZ" onChange={vi.fn()} onSubmit={onSubmit} isLoading={false} />);
  ...
  it('allows Enter on a known ticker once suggestions have loaded', async () => {
    const user = userEvent.setup();
    const onSubmit = vi.fn();
    render(<SearchInput value="AAPL" onChange={vi.fn()} onSubmit={onSubmit} isLoading={false} />);
    await user.click(screen.getByPlaceholderText(/Enter ticker/i));
    // Wait for AAPL to appear in the autocomplete dropdown
    await waitFor(() => expect(screen.getByText('AAPL')).toBeInTheDocument(), { timeout: 500 });
    await user.keyboard('{Enter}');
  ...
  it('clears validation error when the input changes', async () => {
    ...
    const { rerender } = render(
      <SearchInput value="XYZXYZ" onChange={vi.fn()} onSubmit={onSubmit} isLoading={false} />,
    );
    ...
    rerender(<SearchInput value="A" onChange={vi.fn()} onSubmit={onSubmit} isLoading={false} />);
  ...
  it('disables input while loading', () => {
    render(<SearchInput value="" onChange={vi.fn()} onSubmit={vi.fn()} isLoading={true} />);
  ...
  it('does not call onSubmit when value is empty', async () => {
    const user = userEvent.setup();
    const onSubmit = vi.fn();
    render(<SearchInput value="" onChange={vi.fn()} onSubmit={onSubmit} isLoading={false} />);
    await user.click(screen.getByPlaceholderText(/Enter ticker/i));
    await user.keyboard('{Enter}');
...
describe('SearchInput — autocomplete', () => {
  ...
  it('shows dropdown suggestions when focused with a matching prefix', async () => {
    const user = userEvent.setup();
    render(<SearchInput value="AA" onChange={vi.fn()} onSubmit={vi.fn()} isLoading={false} />);
    await user.click(screen.getByPlaceholderText(/Enter ticker/i));
  ...
  it('shows no dropdown for a value with no matches', async () => {
    const user = userEvent.setup();
    render(<SearchInput value="XYZXYZ" onChange={vi.fn()} onSubmit={vi.fn()} isLoading={false} />);
    await user.click(screen.getByPlaceholderText(/Enter ticker/i));
    await waitFor(
      () => expect(global.fetch as ReturnType<typeof vi.fn>).toHaveBeenCalled(),
      { timeout: 500 },
