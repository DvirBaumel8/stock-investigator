...
function TechnicalOutput({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div style={{ fontSize: 13 }}>
      ...
      {(data.ma50 as number | undefined) && <div><b>MA50:</b> ${(data.ma50 as number).toFixed(2)}</div>}
      {(data.ma200 as number | undefined) && <div><b>MA200:</b> ${(data.ma200 as number).toFixed(2)}</div>}
