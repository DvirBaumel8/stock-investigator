...
function setup(isOpen = true, history: HistoryEntry[] = []) {
  ...
  render(
    <Sidebar isOpen={isOpen} onToggle={onToggle} history={history} onSelectTicker={onSelectTicker} onNewResearch={onNewResearch} />,
  );
...
describe('Sidebar — open state', () => {
  ...
  it('calls onToggle when the collapse button is clicked', async () => {
    ...
    await user.click(screen.getByTitle(/Collapse/i));
