...
class MockEventSource {
  ...
  emitNamed(type: string) {
    ...
    else if (listener) (listener as EventListenerObject).handleEvent(new Event(type));
  }
  triggerError() {
    this.onerror?.();
  }
...
// ---------- fetch mock ----------
const ANALYSIS_RESPONSE = { id: 'abc', ticker: 'AAPL', status: 'pending', cached: false, createdAt: '2026-01-01' };
...
describe('useAnalysisStream', () => {
  ...
  it('sets isLoading while waiting for analysis to start', async () => {
    ...
    act(() => { result.current.startAnalysis('AAPL'); });
  ...
  it('populates analysisInfo after POST succeeds', async () => {
    const { result } = renderHook(() => useAnalysisStream());
    await act(async () => { result.current.startAnalysis('AAPL'); });

    await waitFor(() => expect(result.current.analysisInfo?.ticker).toBe('AAPL'));
  ...
  it('appends agent results from SSE messages', async () => {
    ...
    const agentEvent = { id: '1', agentName: 'technical', status: 'completed', output: {}, error: null, durationMs: 500, createdAt: '2026-01-01' };
  ...
  it('updates an existing agent result instead of appending duplicate', async () => {
    ...
    const pending = { id: '1', agentName: 'technical', status: 'pending', output: null, error: null, durationMs: null, createdAt: '2026-01-01' };
    const completed = { ...pending, status: 'completed', output: { rsi14: 55 }, durationMs: 800 };
